...
class Customer(ProgramUser):
    ...
    def register(self, username, password, role):
        ...
        self.AccData = [username, password, role, self.balance]  # put data in AccData to access it later
    ...
    def withdraw_amount(self, amount):  # withdraw func
        if self.balance >= amount:
            self.balance -= (15 / 100) * amount + amount
            modify_line(f"{self.AccData[0]}.txt", 3, self.balance)  # use modify_line function
            print("\n You Withdrew:", amount)
        else:
    ...
    def show_balance(self):  # function to display the balance
        print(f"Welcome {self.AccData[0]} Your Balance is : ", self.balance)
